...
const TransactionDetailScreen = () => {
  ...
  const deleteAndGoBack = async () => {
    ...
    try {
      const stored = await AsyncStorage.getItem('transactions');
      const arr: Transaction[] = stored ? JSON.parse(stored) : [];
      const updated = arr.filter(t => String(t.id) !== String(transaction.id));
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Row icon="description" label="Description" value={transaction.description || '—'} last />
      </View>

      {/* Actions */}
      {!confirm ? (
        <View style={styles.actions}>
          <TouchableOpacity style={[styles.actionBtn, styles.editBtn]} onPress={editTransaction}>
            <Icon name="edit" size={20} color="#1E88E5" />
            <Text style={[styles.actionText, { color: '#1E88E5' }]}>Modifier</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.actionBtn, styles.deleteBtn]} onPress={() => setConfirm(true)}>
        ...
        <View style={styles.confirmBar}>
          ...
          <TouchableOpacity style={[styles.confirmPill, { backgroundColor: '#F0F0F0' }]} onPress={() => setConfirm(false)}>
            <Text style={{ color: '#555' }}>Annuler</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.confirmPill, { backgroundColor: '#FFEBEE' }]} onPress={deleteAndGoBack}>
            <Text style={{ color: '#D32F2F', fontWeight: '700' }}>Supprimer</Text>
          </TouchableOpacity>
        </View>
      )}
    </SafeAreaView>
...
const Row = ({ icon, label, value, last = false }: { icon: string; label: string; value: string; last?: boolean }) => (
...
const styles = StyleSheet.create({
  ...
  card: { borderWidth: 1, borderColor: '#F0F0F0', borderRadius: 12, paddingHorizontal: 12, backgroundColor: '#fff' },
  row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: '#F6F6F6' },
  label: { marginLeft: 8, color: '#757575' },
  value: { color: '#333', fontWeight: '500', flexShrink: 1, textAlign: 'right', marginLeft: 12 },

  actions: { flexDirection: 'row', gap: 12, marginTop: 16 },
  actionBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 14, borderRadius: 10, backgroundColor: '#F5F8FF' },
  actionText: { fontSize: 16, fontWeight: '600' },
  editBtn: { backgroundColor: '#E3F2FD' },
  deleteBtn: { backgroundColor: '#FFEBEE' },

  confirmBar: {
    flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 12, paddingVertical: 12,
    backgroundColor: '#FFF5F5', borderRadius: 10, marginTop: 16, borderWidth: 1, borderColor: '#FFCDD2'
  },
  confirmText: { flex: 1, color: '#D32F2F', fontWeight: '600' },
  confirmPill: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8 },
  backBtn: { marginTop: 16, padding: 10, borderRadius: 8, borderWidth: 1, borderColor: '#E0E0E0' },
});
